...
SetPrvt("_xGrp,_xSub,_xRETORNO,_nUP,_cALIAS,_nRECNO,_nORDEM,_n")
...
cSubGrp	:= Iif(lCopia,SB1->B1_XSUBGRP,M->B1_XSUBGRP)

//-- Tratamento de excessao para MOD - Mao de Obra Direta
If Alltrim(cSubGrp) != "MOD"
	//-- Verifica tamnaho do Campo Grupo para garantir a integridade do sistema
	If Len(AllTrim(cSubGrp)) > nTAM_GRU
		MsgInfo('Esse SubGrupo não pode montar um codigo de produto, verifique o tamanho do codigo.')
		cSubGrp   := " "
		Return(cSubGrp)	
	Endif
	//-- Implemento + 1 no Codigo do Produto
	cUltimo		:= xGetCodGrp(cGrupo,cSubGrp)			//-- Funcao que devolve o ultimo Codigo utilizado no Grupo 
	...
	_cUP		:= AllTrim(cGrupo)+AllTrim(cSubGrp)+Strzero(_nUP,nTAM_COD)
Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function xGetCodGrp(pGrupo,pSubGrp)
